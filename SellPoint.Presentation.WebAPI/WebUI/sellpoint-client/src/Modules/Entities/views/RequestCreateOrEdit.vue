<template>
  <div>
    <h3 class="mx-5 mt-3" v-if="isCreateAction">Crear Peticion</h3>
    <h3 class="mx-5 mt-3" v-if="isShowAction">Consulta de Peticion</h3>
    <h3 class="mx-5 mt-3" v-if="isEditAction">Editar Peticion</h3>
  </div>
...
export default {
  ...
  methods: {
    ...
    async createEntity() {
      ...
      let response = await sellPointApi
        .post("/Request", this.entity)
        .then((resp) => {
          if (resp.status == 200) {
            this.$swal({
              icon: "success",
              title: "Notificación",
              text: "La Peticion ha sigo guardada correctamente.",
            });
            this.clearForm();
          }
        })
        ...
        .then((resp) => {
          ...
          if (resp.status == 200) {
            this.$swal({
              icon: "success",
              title: "Notificación",
              text: "La Peticion ha sigo editada correctamente.",
            });
            this.clearForm();
          }
        })
